...
# * Good with high dimensional data with kernel trick, decision boundary can be linear or non-linear. 
# * Computation is slow when dataset is large. It is also prone to overfitting when dimensionality is high. 
...
# Initialize the classifier
clf = SVC(random_state = 42, verbose = True)

# Create the parameters list 
parameters = {'C': [1, 10], # Large C leads to small margine, it will result in high variance, low bias
              'kernel': ['linear', 'poly', 'rbf'],
              'degree': [1, 3], # for polynomial kernel
              'gamma': [0.001, 0.0001] # kernel coef for rbf, poly, large value tend to overfit 
              }
...
# ##### The optimized SVC performed slightly better than the default one, however, the improvement is not significant. In general, it does not perform better than Gradient Boosting Classifier
